...
import { fetchBadgesStats } from "@/app/actions/users/chartActions";
...
interface Props {
  initialData: BadgesStatsResponse;
}

const BadgesChart = ({ initialData }: Props) => {
  ...
  const [data, setData] = useState<BadgesStatsResponse>(initialData);
  ...
  useEffect(() => {
    const updateData = async () => {
      setLoading(true);
      try {
        const result = await fetchBadgesStats({
  ...
  const badgesStats = data.badgesStats ?? [];
  ...
  return (
    <div className="bg-white flex-col flex gap-5 px-5 sm:px-10 py-6 rounded-3xl shadow-md ">
      <div className="flex flex-col md:flex-row md:justify-between items-start md:items-center gap-3 flex-wrap">
        <h2 className="text-xl font-semibold">{t("badgeStatistics")}</h2>
        <DateRange
          value={dateRange}
      ...
      <div className="h-[400px] w-full rounded-3xl shadow-lg p-4">
        ...
        {!loading &&
        (data.badgesStats ?? data.timeSeriesData ?? []).length > 0 ? (
          <>
            {chartType === "date" && (
              <DateChart data={timeSeriesData} itemKey="badges" />
            )}
            {chartType === "bar" && (
              <BarChartComponent data={badgesStats} valueLabel="users" />
            )}
            {chartType === "pie" && (
              <PieChartComponent data={badgesStats} valueLabel="users" />
            )}
          </>
        ) : (
